    ...
    /*public function store_livraison(Request $request)
{
    ...
    // Trouver la commande
    $commande = Commande::findOrFail($request->commande_id);
    $quantite_totale = $commande->quantite_livre + $livraison->quantite;

    // Vérification des quantités
    if ($quantite_totale > $commande->quantite_commande) {
        return response()->json(['message' => 'vous avez entré un grand nombre']);
    }

    // Mise à jour de la commande et gestion des déchets
    if ($quantite_totale == $commande->quantite_commande) {
        $commande->etat = 'livré';
    } else {
        $commande->etat = 'encours';
    ...
    $commande->quantite_livre = $quantite_totale;
    ...
    $this->gererPaiement($request, $commande, $livraison);

    // Sauvegarde de la commande
    $commande->save();

    return response()->json(['message' => $quantite_totale == $commande->quantite_commande ? 'commande livré' : 'livraison enregistrée']);
}

private function gererPaiement($request, $commande, $livraison)
{
    if ($commande->montant_avance > $request->montant) {
        $commande->montant_avance -= $request->montant;
    } else {
